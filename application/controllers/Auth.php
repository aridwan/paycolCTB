...
class Auth extends CI_Controller {
	...
	public function ajaxDashboard(){
        ...
        $fetch_data = $this->AjaxModel->make_datatables($_POST['tanggal'],$_POST['no_layanan'],$_POST['prioritas'],$_POST['user']);  
        $data = array();  
        ...
        {  
            ...
            if($row->last_update == ''){
            	$sub_array[] = '<a data-toggle="modal" data-target="#detailModal'.$row->id.'">Belum</a>'; 
            } else {
            	$sub_array[] = '<a data-toggle="modal" data-target="#detailModal'.$row->id.'">Sudah</a>'; 
            }

            if ($_SESSION['username']['role'] == 'Administrator'){
            	$pilihan = '<a href="'.base_url('index.php/crud/edit/').$row->id.'"><button type="button" name="update" id="'.$row->id.'" class="btn btn-primary btn-sm">Ubah</button></a>&nbsp
    			...
    			$pilihan = '<a href="'.base_url('index.php/crud/edit/').$row->id.'"><button type="button" name="update" id="'.$row->id.'" class="btn btn-warning btn-sm">Visit</button></a>&nbsp';
    		}

            	$pilihan .='<div class="modal fade" id="myModal'.$row->id.'" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	 <div class="modal-dialog" role="document">
	   <div class="modal-content">
	      ...
	      <div class="modal-footer">
	       ...
	       <a href="'.base_url('index.php/crud/delete/'.$row->id).'">
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        $output = array(  
            ...
            "recordsFiltered"   =>     $this->AjaxModel->get_filtered_data($_POST['tanggal'],$_POST['no_layanan'],$_POST['prioritas'],$_POST['user']),  
            "data"              =>     $data  
        );  
